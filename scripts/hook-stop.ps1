...
.DESCRIPTION
    ...
    - stop_reason 含 error/exception/timeout/network → 警告状态（任务栏整体变黄，持续到问题解决）
    - stop_reason 为其他（正常结束）→ 完成状态（绿色进度条，用户聚焦窗口后 1 秒消失）
...
if ($stopReason -match "error|Error|ERROR|exception|timeout|network") {
    ...
    # 正常完成 → 绿色进度条，用户打开窗口后 1 秒自动消失
    & "$scriptsDir\taskbar-overlay.ps1" -Status complete
}
